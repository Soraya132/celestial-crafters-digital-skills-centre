  ...
  <section className={`px-5 py-14 md:px-10 bg-gray-300 `}>
    ...
    >
      ...
      <p className="py-5 font-normal  text-[20px] text-center ">
        ...
        is dedicated to providing digital skills training to vulnerable girls
        and young women.
      </p>
      <div className="mt-[50px] flex flex-col gap-[35px]">
        {insights.map((item, index) => (
          <SkillCard key={`insight-${index}`} {...item} index={index + 1} />
        ))}
      </div>
    </motion.div>
  </section>
...
export default DigitalSkillsCenter;
